...
public class Statistiche {
	...
	public Map<Formica, Integer> raccoltoPerFormica(Set<Cibo> raccolto) {
		...
		Map<Formica, Integer> map = new HashMap<>();
		for(Cibo a : raccolto) {
			if(!map.containsKey(a.getRaccoglitrice())) {
					map.put(a.getRaccoglitrice(), 1);
			}
			else {
				int c = map.get(a.getRaccoglitrice())+1 ;
				map.put(a.getRaccoglitrice(), c);
			}
	...
	public Map<Class<?>, Set<Cibo>> raccoltoPerTipoDiFormica(Set<Cibo> raccolto) {
		...
		Map<Class<?>, Set<Cibo>> map = new HashMap<>();
		
		for(Cibo a: raccolto) {
			if(!map.containsKey(a.getRaccoglitrice().getClass())) {
				Set<Cibo> set = new HashSet<>();
				set.add(a);
				map.put(a.getRaccoglitrice().getClass(),set );
			}
			map.get(a.getRaccoglitrice().getClass()).add(a);
		}
	...
	public List<Class<?>> ordinaStrategieDiRaccolta(final Map<Class<?>, Set<Cibo>> c2c) {
		...
		listaOrdinata.addAll(c2c.keySet());
		Collections.sort(listaOrdinata, new comparatore(c2c));
